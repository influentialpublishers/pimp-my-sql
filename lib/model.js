...
const save = R.curry(
  (intercepts, sql_factory, conn, request, current = null) => {
    ...
    let _save    = method(intercepts, sql_factory, conn)

    if (current) _save = _save(current)
    ...
    const retrieve = function retrieve(insertId) {

      const id = current ? current.id : insertId

      return getById(sql_factory, conn, id, NO_CACHE)
...
const filter = R.curry((sql_factory, map, conn, params) => {
  params = Search.parseParams(map)(params)
  ...
  return query.select(conn, sql, [], false)
})
